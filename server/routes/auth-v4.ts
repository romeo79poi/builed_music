...
router.post(
  "/verification/email/verify-token",
...
router.post(
  "/verification/email/validate-token",
...
if (process.env.NODE_ENV === "development") {
  ...
  router.post("/dev/generate-verification-link", async (req, res) => {
    try {
  ...
  router.post("/dev/generate-reset-link", async (req, res) => {
    try {
      const { email, userId } = req.body;
