  ...
  sendPerformanceMetrics() {
    ...
    this.port.postMessage({
      ...
      data: {
        ...
        efficiency: (this.processedSamples / this.sampleRate) / (elapsedMs / 1000) * 100
      }
    });
  }
  ...
  process(inputs, outputs, parameters) {
    ...
    try {
      ...
      for (let channel = 0; channel < Math.min(input.length, output.length); channel++) {
        ...
        const inputVector = new this.wasmModule.VectorFloat(Array.from(inputChannel));
      ...
      if (this.processedSamples % (this.sampleRate * 2) === 0) { // Every 2 seconds
        this.sendPerformanceMetrics();
      }
