...
const ResetPassword: React.FC = () => {
  ...
  const validateToken = async (token: string) => {
    ...
    try {
      const response = await fetch('/api/v4/auth/password/validate-reset-token', {
        method: 'POST',
        headers: {
      ...
      <div className="min-h-screen bg-gradient-to-br from-purple-darker via-background to-purple-dark flex items-center justify-center py-8 px-4">
        <Card className="w-full max-w-md">
          <CardHeader className="text-center">
            ...
            <CardTitle className="text-2xl font-bold">Validating Reset Token...</CardTitle>
            <CardDescription>
                ...
                {newPassword && (
                  <div className="mt-2">
                    ...
                    <div className="mt-1 text-xs text-muted-foreground space-y-1">
                      <div className={newPassword.length >= 8 ? 'text-green-600' : ''}>
                        ✓ At least 8 characters
                      </div>
                      <div className={/[A-Z]/.test(newPassword) ? 'text-green-600' : ''}>
                        ✓ One uppercase letter
                      </div>
                      <div className={/[a-z]/.test(newPassword) ? 'text-green-600' : ''}>
                        ✓ One lowercase letter
                      </div>
                      <div className={/\d/.test(newPassword) ? 'text-green-600' : ''}>
                        ✓ One number
                      </div>
                      <div className={/[!@#$%^&*(),.?":{}|<>]/.test(newPassword) ? 'text-green-600' : ''}>
