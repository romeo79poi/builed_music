...
interface FastAudioControls {
  ...
  crossfadeTracks: (track1: Float32Array, track2: Float32Array, fadeRatio: number) => Float32Array;
...
export function useFastAudio(): FastAudioControls {
  ...
  useEffect(() => {
    if (!initializationAttempted.current) {
      ...
      fastAudioEngine.initialize()
        .then(() => {
          setIsEngineReady(true);
          console.log('🚀 Fast C++ Audio Engine ready for high-performance processing');
        })
        .catch(error => {
          console.warn('⚠️ Fast audio engine initialization failed, using JavaScript fallback:', error);
  ...
  const setEqualizerBand = useCallback((band: number, gain: number) => {
    if (isEngineReady) {
      fastAudioEngine.setEqualizerBand(band, gain);
    }
  }, [isEngineReady]);

  const processAudioBuffer = useCallback((buffer: Float32Array): Float32Array => {
    if (isEngineReady) {
  ...
  const getSpectrumData = useCallback((buffer: Float32Array, fftSize: number = 512): Float32Array => {
    if (isEngineReady) {
...
export function useAudioSpectrum(audioElement: HTMLAudioElement | null, fftSize: number = 512) {
  const [spectrum, setSpectrum] = useState<Float32Array>(new Float32Array(fftSize / 2));
    ...
    try {
      ...
      const updateSpectrum = () => {
        if (analyzerRef.current) {
          ...
          const enhancedSpectrum = fastAudio.getSpectrumData(dataArray, fftSize);
