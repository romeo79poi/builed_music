  ...
  if (!response.ok) {
    ...
    if (!error) {
      ...
      if (data && typeof data === 'object') {
        error = data.message || data.error || data.detail || response.statusText;
...
export const apiGet = <T = any>(url: string, options: Omit<ApiRequestOptions, 'method'> = {}) =>
  safeApiRequest<T>(url, { ...options, method: 'GET' });

export const apiPost = <T = any>(url: string, body?: any, options: Omit<ApiRequestOptions, 'method' | 'body'> = {}) =>
  safeApiRequest<T>(url, { ...options, method: 'POST', body });

export const apiPut = <T = any>(url: string, body?: any, options: Omit<ApiRequestOptions, 'method' | 'body'> = {}) =>
  safeApiRequest<T>(url, { ...options, method: 'PUT', body });

export const apiDelete = <T = any>(url: string, options: Omit<ApiRequestOptions, 'method'> = {}) =>
  safeApiRequest<T>(url, { ...options, method: 'DELETE' });

// Auth-specific helper
export const authApiRequest = <T = any>(url: string, options: ApiRequestOptions = {}) => {
