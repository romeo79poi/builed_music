...
export async function testAllSettings(firebaseUserId: string) {
  console.log(
    "🧪 Testing all settings functionality for user:",
    firebaseUserId,
  );
  ...
  try {
    ...
    const bulkUpdate = await firebaseSettingsService.updateSettings(
      firebaseUserId,
      {
        theme: "dark",
        playback: {
    ...
    const resetSuccess =
      await firebaseSettingsService.resetSettings(firebaseUserId);
...
export async function testSettingButton(
