...
const isValidUrl = (url: string) => {
  try {
    new URL(url);
    return (
      url.includes(".supabase.co") && !url.includes("[") && !url.includes("]")
    );
  } catch {
    return false;
  }
...
export const supabase = (() => {
  if (
    ...
    console.warn(
      "⚠️ Supabase not configured. Using mock client. Please set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY environment variables.",
    );
