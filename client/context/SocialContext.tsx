...
export const SocialProvider: React.FC<SocialProviderProps> = ({ children }) => {
  ...
  const loadSocialData = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "⚠️ Firestore followers permission denied, using empty data",
        );
        setFollowers([]);
      }
      ...
      try {
        ...
        console.warn(
          "⚠️ Firestore following permission denied, using empty data",
        );
        setFollowing([]);
      }
      ...
      try {
        ...
        console.warn(
          "⚠️ Firestore notifications permission denied, using empty data",
        );
        setNotifications([]);
      }
    } catch (err: any) {
  ...
  const setupRealtimeListeners = () => {
    ...
    try {
      ...
      const unsubscribeFollowers = onSnapshot(
        ...
        (error) => {
          console.warn(
            "⚠️ Firestore followers listener error, ignoring:",
            error.message,
          );
        },
      );
    ...
    try {
      ...
      const unsubscribeFollowing = onSnapshot(
        ...
        (error) => {
          console.warn(
            "⚠️ Firestore following listener error, ignoring:",
            error.message,
          );
        },
      );
    ...
    try {
      ...
      const unsubscribeNotifications = onSnapshot(
        ...
        (error) => {
          console.warn(
            "⚠️ Firestore notifications listener error, ignoring:",
            error.message,
          );
        },
      );
  ...
  const followUser = async (
    ...
    try {
      ...
      const parsedUserData = currentUserData
      ...
      try {
        ...
        console.warn(
          "⚠️ Cannot save notification to Firestore, permission denied",
        );
  ...
  const unfollowUser = async (userId: string): Promise<boolean> => {
    ...
    try {
      ...
      const followRelationship = following.find(
        (f) => f.followingId === userId && f.isActive,
      );
      ...
      try {
        ...
        console.warn(
          "⚠️ Cannot update Firestore, permission denied. Updating locally.",
        );
  ...
  const getMutualFollowers = async (userId: string): Promise<SocialUser[]> => {
    ...
    try {
      ...
      const targetUserFollowingSnapshot = await getDocs(
        targetUserFollowingQuery,
      );
      const targetUserFollowing = targetUserFollowingSnapshot.docs.map(
      ...
      for (const userId of mutualFollowIds.slice(0, 10)) {
        // Limit to 10
        const followRelationship = following.find(
  ...
  const markNotificationAsRead = async (
    ...
    try {
      ...
      console.warn(
        "⚠️ Cannot update notification, permission denied. Updating locally.",
      );
      // Update local state
